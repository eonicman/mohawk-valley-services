// Directory-site Worker: lead capture + DIY feature backend + premium listings + static assets.
...
//   GET  /guides/*.html            -> static guide page with the #diy-grid quick-fix cards
//                                      server-rendered from real D1 data (same technique as
//                                      /category/*), so crawlers that don't run diy-dynamic.js's
//                                      client-side fetch see real content, not a "Loading
//                                      guides…" shell (AdSense "low value content" fix, 2026-08-07)
//   POST /api/premium-checkout     -> upsert a `businesses` row, create a Stripe Checkout Session,
//                                      return {url} for the client to redirect to
//   POST /stripe-webhook           -> verify Stripe signature, activate/cancel premium on
...
//   GET  / , /aria                 -> static page, but with the #aria-video-embed block
//                                      rewritten to the ad video matching the visitor's
//                                      selected language (site_lang cookie, set by the
//                                      language selector on those two pages) -- language
//                                      selection is worker-side so there's no flash of
//                                      the wrong-language ad on load
//   everything else                -> static assets (env.ASSETS), unchanged
// D1 binding: env.LEADS -> database "directory-leads" (tables: leads, diy_*, businesses, users,
//   user_sessions, saved_guides).

const SITE_HOST = "mohawkvalleyservices.com";
const SITE_NAME = "Mohawk Valley Services";
const guideUrl = (cat, slug) => `https://mohawkvalleyservices.com/guides/${cat}.html#${slug}`;
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (/^\/category\/[a-z0-9-]+(\/[a-z0-9-]+)?(\.html)?$/.test(p) && request.method === "GET") {
        return renderCategoryPage(request, env, url);
      }
      if (/^\/guides\/[a-z0-9-]+(\.html)?$/.test(p) && request.method === "GET") {
        return renderGuidePage(request, env, url);
      }
      if ((p === "/" || p === "/aria") && request.method === "GET") {
...
// Shared by the /api/diy-guides JSON endpoint (diy-dynamic.js's client fetch) and
// renderGuidePage's server-side render below, so the two never drift on ordering/filtering.
async function fetchGuides(env, host, category, limit) {
  const rows = await env.LEADS.prepare(
    `SELECT g.*, COALESCE(r.n, 0) AS request_count
     ...
     LIMIT ?`
  ).bind(host, host, category, limit).all();
  return (rows.results || []).map(guideRow);
}
...
async function diyGuidesList(request, env, url) {
  ...
  const guides = await fetchGuides(env, url.host, category, limit);
  return json({ status: "ok", category, guides });
}
...
/* ---------- premium listings: dynamic category rendering ---------- */
...
function bizCardHtml(b) {
  ...
  const website = b.website
    ? `<div style="margin-top:.4rem;"><a href="${escapeHtml(b.website)}" target="_blank" rel="noopener sponsored" style="color:#3e6b3e;text-decoration:none;font-size:.85rem;font-weight:500;">\u{1F310} ${escapeHtml(b.website.replace(/^https?:\/\//, "").replace(/\/$/, ""))}</a></div>`
    : "";
...
async function renderCategoryPage(request, env, url) {
  ...
  const rows = townSlug
    ? await env.LEADS.prepare(
        `SELECT name, category, phone, address, website, premium, town FROM businesses
    ...
    : await env.LEADS.prepare(
        `SELECT name, category, phone, address, website, premium FROM businesses
  ...
  if (townSlug) {
    ...
    introHtml = `<p style="margin-bottom:1rem;color:#555;">Looking for ${escapeHtml(category)} services in ${escapeHtml(townName)}? Browse verified local businesses below.</p>`;
  } else {
...
/* ---------- guide pages: server-render the #diy-grid quick-fix cards ----------
   Ports diy-dynamic.js's renderCard/renderTool/renderStep to the server so every
   /guides/*.html response already contains real guide content, not the
   "Loading guides…" placeholder that only fills in once diy-dynamic.js's client-side
   fetch to /api/diy-guides runs. That client fetch is invisible to crawlers that don't
   execute it -- notably Google's AdSense content crawler -- which is what triggered
   the "low value content" flag on this site (2026-08-05, ~30min after AdSense ads
   were added to these pages in eda2e03). diy-dynamic.js still re-fetches and
   re-renders on top of this for interactive users (harmless, same data), but
   crawlers and anyone without JS now see the real thing immediately. */
function renderToolHtml(t) {
  return `<span class="diy-tool">${t.emoji || ""} <a href="${escapeHtml(t.url)}" target="_blank" rel="nofollow sponsored">${escapeHtml(t.label)}</a></span>`;
}

function renderStepHtml(s, i) {
...
function guideCardHtml(g) {
  const tools = (g.tools || []).map(renderToolHtml).join("");
  const hasSteps = (g.steps || []).length > 0;
  const steps = (g.steps || []).map(renderStepHtml).join("");
  const difficulty = g.difficulty || "easy";
  const diffColor = difficulty === "hard" ? "#ef4444" : difficulty === "medium" ? "#f59e0b" : "#22c55e";
  return `
    <div class="diy-card collapsed" data-difficulty="${escapeHtml(difficulty)}" data-slug="${escapeHtml(g.slug)}">
      <div class="diy-card-header">
        <div class="diy-icon" style="background:${g.icon_bg || "#f0f0f0"};">${g.icon_emoji || "🔧"}</div>
        <div>
          <h3>${escapeHtml(g.project)}</h3>
          <div class="diy-meta">
            <span class="diy-difficulty ${escapeHtml(difficulty)}" style="color:${diffColor}">● ${difficulty.charAt(0).toUpperCase() + difficulty.slice(1)}</span>
      ...
      <div class="diy-card-body">
        ...
        <div class="diy-actions">
          ...
          ${g.shop_link ? `<a href="${escapeHtml(g.shop_link)}" class="diy-btn diy-btn-secondary" target="_blank" rel="nofollow sponsored">${escapeHtml(g.shop_label || "🛒 Shop")}</a>` : ""}
        </div>
      </div>
...
async function renderGuidePage(request, env, url) {
  const category = url.pathname.replace(/^\/guides\//, "").replace(/\.html$/, "");
  const assetUrl = new URL(url);
  assetUrl.pathname = `/guides/${category}`;
  const resp = await env.ASSETS.fetch(new Request(assetUrl, request));
  if (!resp.ok) return resp;
  const html = await resp.text();

  // Pages with no #diy-grid (a hand-authored long-form guide page with no quick-fix
  // widget) pass through unchanged, same fallback pattern as renderCategoryPage above.
  const gridMatch = /<div class="diy-grid" id="diy-grid"([^>]*)>/.exec(html);
  if (!gridMatch) return new Response(html, { status: resp.status, headers: resp.headers });
  const startIdx = gridMatch.index;
  const endIdx = findDivBlockEnd(html, startIdx);
  if (endIdx === -1) return new Response(html, { status: resp.status, headers: resp.headers });

  const limitMatch = /data-limit="(\d+)"/.exec(gridMatch[1]);
  const limit = limitMatch ? Math.min(parseInt(limitMatch[1], 10) || 10, 25) : 10;

  const guides = await fetchGuides(env, url.host, category, limit);
  const cardsHtml = guides.length
    ? guides.map(guideCardHtml).join("\n")
    : `<p style="color:#888;">No guides yet for this category — be the first to request one below.</p>`;

  const openTag = html.slice(startIdx, startIdx + gridMatch[0].length);
...
async function premiumCheckout(request, env, url) {
  ...
  const upsert = await env.LEADS.prepare(
